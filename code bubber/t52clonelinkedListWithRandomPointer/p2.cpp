...
class Solution
{
    ...
    Node *copyList(Node *head)
    {
        ...
        Node* cloneTail=NULL;
        while(mainHead!=NULL){
            int data=mainHead->data;
            insertAtTail(cloneHead,cloneTail,data);
            mainHead=mainHead->next;
        }
        
        // cloneNodes add in between orignal list
        Node* oglist=head;
        Node* clonelist=cloneHead;
        while(oglist!=NULL && clonelist!=NULL){
            Node* next=oglist->next;
            oglist->next=clonelist;
            oglist=next;
            
            next=clonelist->next;
            clonelist->next= oglist;
            clonelist=next;
        }
        ...
        clonelist=cloneHead;
        while(oglist!=NULL && clonelist!=NULL){
            oglist->next = clonelist->next;
            oglist=oglist->next;
            
            if (oglist!=NULL){
                clonelist->next = oglist->next;
            }
